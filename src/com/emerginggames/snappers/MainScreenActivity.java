...
public class MainScreenActivity extends Activity {
    private static final long INTERVAL_TO_ASK_TO_RATE = 3 * 24 * 60 * 60 * 1000;
...
/*        lp = findViewById(R.id.logoCont).getLayoutParams();
    ...
    void tryToAskToRateApp(){
        UserPreferences prefs =UserPreferences.getInstance(getApplicationContext());
        long lastAsked = prefs.getLastAskAppRate();
        long now = System.currentTimeMillis();
        if (lastAsked == 0){
            prefs.setLastAskedToRateApp(now);
            return;
        }

        if (now - lastAsked > INTERVAL_TO_ASK_TO_RATE){
            prefs.setLastAskedToRateApp(now);
