	...
	private class CheckForVirtualGoodsTask extends AsyncTask<Void, Void, ArrayList<VGStoreItem>>
	{
		...
		protected void onPostExecute(ArrayList<VGStoreItem> result)
		{
			if (result != null)
			{
                ...
                TJCVirtualGoods.TapjoyDownloadListener listener = TJCVirtualGoods.getVirtualGoodDownloadListener();

                listener.onDownloadedItems(purchasedItemArray);
				...
				for (int i = 0; i < purchasedItemArray.size(); i++)
				{
					...
					// && !vgItem.getDatafileUrl().equals(""))

					// Download any virtual good even if there is no data.
					if (vgItem != null)
					{
